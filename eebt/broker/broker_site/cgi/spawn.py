...
class WSHdl:
    def __init__(self, ws_key, session_key):
        ...
        if res['operation'] == 'identify':
            rep = json.dumps({"operation": "identify", "side": "cgi", "ws_key_browser": self.ws_key_browser,
                              "cgi_key": CGI_KEY})
            print(rep)
            self.ws.send(rep)
    ...
    def send_msg(self, msg):
        self.ws.send(
            json.dumps({"operation": "update", "ws_key_browser": self.ws_key_browser, "msg": msg, "cgi_key": CGI_KEY}))
        print(
            json.dumps({"operation": "update", "ws_key_browser": self.ws_key_browser, "msg": msg, "cgi_key": CGI_KEY}))

    def send_finish(self):
        self.ws.send(
            json.dumps({"operation": "finish", "ws_key_browser": self.ws_key_browser, "cgi_key": CGI_KEY}))
        print(
            json.dumps({"operation": "finish", "ws_key_browser": self.ws_key_browser, "cgi_key": CGI_KEY}))
...
class AWSHdl:
    ...
    def add_user(self, instance_id):
        ...
        quota_bytes = QUOTA
        alive_for = "+%d" % int(VALID_PERIOD / 60)
    ...
    def assign_instance(self, instance_id, u_p=None):
        ...
        if self.wshdl is not None:
            ...
            valid_until = current_time + VALID_PERIOD
            url = "http://127.0.0.1:12346/?user=%s?pass=%s?host=%s?port=10182?type=tcp?valid=%f?quota=%d?origin=" % (
            u, p, ip, valid_until, QUOTA)
            self.wshdl.send_msg(json.dumps({'user': u, 'password': p, 'host': ip, 'url': url}))
            self.wshdl.send_finish()
